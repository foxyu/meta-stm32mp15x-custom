...
CWD=$(pwd)
METALAYER=meta-stm32mp1-bsp-base
...
LIST_IMAGES=$(ls sources/*/recipes-samples/images/*.bb | sed s/\.bb//g | sed -r 's/^.+\///' | xargs -I {} echo -e "\t"{})
...
if [ "x$MACHINE" = "x" ] || [ "$VALID_MACHINE" = "0" ]; then
    ...
    echo -e "\n\nThese are the supported machines:\n$LIST_MACHINES\n\n"
    return 1
else
...
# Create an eula file
if [ ! -e $(pwd)/sources/meta-st-stm32mp/conf/eula/stm32mp1-discotest ]; then
    echo "Copying the EULA for ${MACHINE}"
    cp $(pwd)/sources/${METALAYER}/conf/eula/stm32mp1-discotest $(pwd)/sources/meta-st-stm32mp/conf/eula/
fi
...
if [ ! -e conf/local.conf.sample ]; then
    ...
    cp -f $CWD/sources/${METALAYER}/conf/bblayers.conf $CWD/$1/conf/bblayers.conf
    cp -f $CWD/sources/${METALAYER}/conf/local.conf $CWD/$1/conf/local.conf
...
echo -e "\nThese are the supported distros:\n${LIST_DISTROS}"
cat <<EOF
You can now build your image. To build the stm32mp1-qt-eglfs-image then run this:
